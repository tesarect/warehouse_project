...
# _loading_position = [4.3563, -0.7774, -0.71781, 0.69623]
    # position:
    #   x: 4.381706294839297
    #   y: -0.4222988595639691
    #   z: 0.0
    # orientation:
    #   x: 0.0
    #   y: 0.0
    #   z: -0.7290712347103127
    #   w: 0.6844378238510641

_loading_position = [4.3817, -0.42230, -0.72907, 0.684437]
...
# Locate robot_config package
pkg_path = get_package_share_directory('robot_config')

# Load YAML manually
param_file = os.path.join(pkg_path, 'config', 'real.yaml')
...
def load_parameters():
        param_file = os.path.join(pkg_path, 'config', 'robot.yaml')
        ...
        # Declare + set params in the node
        for key, value in params.items():
            shelf_attacing_node.declare_parameter(key, value)

        # Now you can use them
        odom_frame = shelf_attacing_node.get_parameter('odom_frame').value
        base_frame = shelf_attacing_node.get_parameter('base_frame').value
        cmd_vel_topic = shelf_attacing_node.get_parameter('cmd_vel_topic').value

        shelf_attacing_node.get_logger().info(f"Loaded odom_frame = {odom_frame}")
        shelf_attacing_node.get_logger().info(f"Loaded base_frame = {base_frame}")
        shelf_attacing_node.get_logger().info(f"Loaded cmd_vel_topic = {cmd_vel_topic}")
